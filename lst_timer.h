...
public:
  ...
  void add_timer(util_timer *timer) {
     ...
     * 则把该定时器插入链表头部,
     * 作为链表新的头节点。否则就需要调用重载函数add_timer(util_timer*timer,util_timer*lst_head),
  ...
  /*当某个定时任务发生变化时,
   * 调整对应的定时器在链表中的位置。这个函数只考虑被调整的定时器的超时时间延长的情况,
   * 即该定时器需要往链表的尾部移动*/
...
private:
  /*一个重载的辅助函数,
   * 它被公有的add_timer函数和adjust_timer函数调用。该函数表示将目标定时器timer添加到节点lst_head之后的部分链表中*/
